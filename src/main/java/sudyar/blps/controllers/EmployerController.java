...
public class EmployerController {
    ...
    public InfoResponse sendMessage(@RequestBody MessageRequest messageRequest) {
        ...
        if (userService.exitsUserLogin(login)) {
            int id = messageRequest.getIdTargetOrdering();
            if (orderService.existById(id)) {
                noticeService.createNotice(messageRequest,
                        SecurityContextHolder.getContext().getAuthentication().getName(), Status.MESSAGE);
                return new InfoResponse("Сообщение отправлено", 0);
            } else return new InfoResponse("Заказа с id '" + id + "' нет", 1);
        } else return new InfoResponse("Пользователя с логином '" + login + "' нет", 1);
    }
    @DeleteMapping("/deleteMessage")
    public void deleteMessage(@RequestBody int idMessage){
        noticeService.deleteMessage(idMessage);
    }
